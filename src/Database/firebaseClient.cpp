...
float FirebaseClient::getFloatValue(const std::string& url) {
    ...
    if (curl) {
        ...
        try {
            ...
            std::cerr << "Error converting response to float: " << ia.what() << std::endl;
        }
        catch (const std::out_of_range& oor) {
            std::cerr << "Response out of range for float: " << oor.what() << std::endl;
        }
    }
...
nlohmann::json FirebaseClient::getJSONValue(const std::string& url) {
    ...
    try {
        ...
        std::cerr << "Response received: " << response << std::endl; // Depuração
        return nullptr;
    }
}
